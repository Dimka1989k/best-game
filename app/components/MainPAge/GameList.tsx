  ...
  return (
    <div className="grid grid-cols-2 grid-rows-2 gap-6 max-md:gap-4">
      {GAME_LIST.map((game) => (
        <Card
          key={game.id}
          className="w-full max-w-74 h-90.5 max-sm:max-h-57.5 object-contain bg-center flex flex-col p-6
                     max-md:pt-2.5 max-md:pb-4 max-md:px-2.5
                     radius-lg shadow-cards-games"
          style={{ backgroundImage: `url(${game.background})` }}
        >
          <CardHeader>
            <div
              className={`shadow-green flex items-center justify-center
                          bg-[linear-gradient(116.24deg,#B9FF58_-6.88%,#69B400_83.61%)]
                          ${game.badgeWidthClass}
                          max-sm:h-6.5 h-7 radius-pill
                          text-white text-inter-bold max-md:text-sm!`}
            >
              {game.badge}
            </div>
          </CardHeader>
      ...
      backdrop-blur-lg
      pointer-events-none
      mask-[linear-gradient(to_bottom,transparent_47.37%,black_100%)]
      [-webkit-mask-image:linear-gradient(to_bottom,transparent_47.37%,black_100%)] rounded-b-lg
    "
            />
